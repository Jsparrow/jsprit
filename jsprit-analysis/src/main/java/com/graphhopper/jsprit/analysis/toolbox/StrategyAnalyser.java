...
public class StrategyAnalyser implements AlgorithmEndsListener, StrategySelectedListener, IterationStartsListener {
    ...
    public void informSelectedStrategy(SearchStrategy.DiscoveredSolution discoveredSolution, VehicleRoutingProblem vehicleRoutingProblem, Collection<VehicleRoutingProblemSolution> vehicleRoutingProblemSolutions) {
        ...
        strategyMap.putIfAbsent(strategyId, new Strategy(strategyId));
    ...
    public void informAlgorithmEnds(VehicleRoutingProblem problem, Collection<VehicleRoutingProblemSolution> solutions) {
        ...
        try {
            for (String stratId : strategyMap.keySet()) {
                ...
                out.write(new StringBuilder().append("id: ").append(stratId).append(", #selected: ").append(strategy.getCountSelected()).append(", #newSolutions: ").append(strategy.getCountNewSolution())
						.append(", #improvedSolutions: ").append(strategy.getCountImproved()).append(", improvements: ").append(strategy.getImprovements().toString()).append("\n").toString());
            }
        } catch (IOException e) {
