...
public class SolutionAnalyser {
	...
	private Double twViolation;

	private Capacity capViolation;
	...
	private void recalculateSolutionIndicators() {
        solution.getRoutes().forEach(route -> {
            ...
            tpDistance += getDistance(route);
            tpTime += getTransportTime(route);
            waiting_time += getWaitingTime(route);
            serviceTime += getServiceTime(route);
            operationTime += getOperationTime(route);
            twViolation += getTimeWindowViolation(route);
            capViolation = Capacity.addup(capViolation, getCapacityViolation(route));
            fixedCosts += getFixedCosts(route);
            variableTransportCosts += getVariableTransportCosts(route);
        ...
        totalCosts = solutionCostCalculator.getCosts(this.solution);
    }
	...
	private void clearSolutionIndicators() {
        ...
        operationTime = 0.;
        twViolation = 0.;
        capViolation = Capacity.Builder.newInstance().build();
        fixedCosts = 0.;
        variableTransportCosts = 0.;
        totalCosts = 0.;
    ...
    private static class LoadAndActivityCounter implements StateUpdater, ActivityVisitor {
        ...
        private StateId pickupCountId;

        private StateId pickupAtBeginningCountId;

        private StateId deliveryCountId;

        private StateId deliveryAtEndCountId;

        private StateId loadPickedId;

        private StateId loadDeliveredId;

        private VehicleRoute route;

        private LoadAndActivityCounter(StateManager stateManager) {
            this.stateManager = stateManager;
            pickupCountId = stateManager.createStateId(PICKUP_COUNT);
            deliveryCountId = stateManager.createStateId(DELIVERY_COUNT);
            loadPickedId = stateManager.createStateId(LOAD_PICKED);
            loadDeliveredId = stateManager.createStateId(LOAD_DELIVERED);
            pickupAtBeginningCountId = stateManager.createStateId(PICKUP_COUNT_AT_BEGINNING);
            deliveryAtEndCountId = stateManager.createStateId(DELIVERY_COUNT_AT_END);
        }
        ...
        @Override
        public void finish() {
            stateManager.putRouteState(route, pickupCountId, pickupCounter);
            stateManager.putRouteState(route, deliveryCountId, deliveryCounter);
            stateManager.putRouteState(route, loadPickedId, pickedUp);
            stateManager.putRouteState(route, loadDeliveredId, delivered);
            stateManager.putRouteState(route, pickupAtBeginningCountId, pickupAtBeginningCounter);
            stateManager.putRouteState(route, deliveryAtEndCountId, deliverAtEndCounter);
        }
    }
    ...
    private static class BackhaulAndShipmentUpdater implements StateUpdater, ActivityVisitor {
        ...
        public void finish() {
			...
			openShipments.values().forEach(act -> {
