...
//         problemStates.putState(stateId, type, state);
	...
	private ClassCastException getClassCastException(ClassCastException e, StateId stateId, String requestedTypeClass, String memorizedTypeClass) {
        return new ClassCastException(new StringBuilder().append(e).append("\n").append("state with stateId '").append(stateId.toString()).append("' is of ").append(memorizedTypeClass)
				.append(". cannot cast it to ").append(requestedTypeClass).append(".").toString());
    }
...
//        if (route.isEmpty()) return null;
    ...
    public <T> void putActivityState(TourActivity act, StateId stateId, T state) {
        ...
        if (stateId.getIndex() < initialNoStates) {
			throw new IllegalStateException(new StringBuilder().append("either you use a reserved stateId that is applied\n").append("internally or your stateId has been created without index, e.g. StateFactory.createId(stateName)\n").append(" does not assign indeces thus do not use it anymore, but use\n ").append("stateManager.createStateId(name)\n").append(" instead.\n").toString());
		}
...
//        if (route.isEmpty()) return;
        if(isIndexedBased){
            ...
            routeStateMap.putIfAbsent(route, new Object[stateIndexCounter]);
...
//        if (route.isEmpty()) return;
        if(isIndexedBased){
            ...
            vehicleDependentRouteStateMap.putIfAbsent(route, new Object[nuVehicleTypeKeys][stateIndexCounter]);
	...
	public void addAllStateUpdater(Collection<StateUpdater> updaters) {
        updaters.forEach(this::addStateUpdater);
    }
...
//		log.debug("insert " + job2insert + " in " + inRoute);
        ...
        routeVisitors.forEach(v -> v.visit(inRoute));
    ...
    public void informInsertionStarts(Collection<VehicleRoute> vehicleRoutes, Collection<Job> unassignedJobs) {
        ...
        vehicleRoutes.forEach(route -> {
            routeVisitors.forEach(v -> v.visit(route));
	...
	public void reCalculateStates(VehicleRoute route){
        informInsertionStarts(Collections.singletonList(route),Collections.<Job>emptyList());
    }
