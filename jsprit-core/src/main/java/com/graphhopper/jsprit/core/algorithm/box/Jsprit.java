...
public class Jsprit {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Jsprit.class);
	...
	private VehicleRoutingAlgorithm create(final VehicleRoutingProblem vrp) {
        ...
        boolean condition = noThreads > 1 && es == null;
		if (condition) {
        ...
        customStrategies.keySet().forEach(customStrategy -> prettyBuilder.withStrategy(customStrategy, customStrategies.get(customStrategy)));
	...
	private void handleExecutorShutdown(VehicleRoutingAlgorithm vra) {
		...
		final Thread hook = new Thread() {
			...
			public void run() {
				...
				logger.error("shutdownHook shuts down executorService");
				es.shutdown();
		    }
		};
		Runtime.getRuntime().addShutdownHook(hook);
	...
	private SolutionCostCalculator getObjectiveFunction(final VehicleRoutingProblem vrp, final double maxCosts) {
        ...
        SolutionCostCalculator solutionCostCalculator = (VehicleRoutingProblemSolution solution) -> {
		    ...
		    for (VehicleRoute route : solution.getRoutes()) {
		        ...
		        boolean condition = route.getVehicle().getBreak() != null && !hasBreak && route.getEnd().getArrTime() > route.getVehicle().getBreak().getTimeWindow().getEnd();
